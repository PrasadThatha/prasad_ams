...
def parse_resume(text):
    ...
    resume = {
        ...
        "sidebar_sections": [], "summary": [], "experience": []
    }
    ...
    if exp_lines:
        ...
        date_pattern = re.compile(
            ...
            r'|(\d{4}\s*[-–]\s*(\d{4}|Present|present))', re.IGNORECASE)
...
def generate_pdf(resume, output_path):
    ...
    getSampleStyleSheet()
    sidebar_heading = ParagraphStyle('sidebar_heading', fontName='Helvetica-Bold', fontSize=13, leading=16, textColor=colors.white, spaceAfter=10)
    sidebar_text = ParagraphStyle('sidebar_text', fontName='Helvetica', fontSize=9, leading=18, textColor=colors.white)
    section_heading = ParagraphStyle('section_heading', fontName='Helvetica-Bold', fontSize=15, leading=18, textColor=colors.HexColor("#333333"), spaceAfter=14)
    bullet_style = ParagraphStyle('bullet_style', fontName='Helvetica', fontSize=9, leading=18, leftIndent=15, textColor=colors.black)
    company_style = ParagraphStyle('company_style', fontName='Helvetica-Bold', fontSize=10, leading=14, textColor=colors.black, spaceAfter=8)
    def draw_first_page(canvas, doc):
        ...
        canvas.drawString(125, PAGE_HEIGHT - 88, f"Mob: {resume['mobile']} | Email: {resume['email']}")
        canvas.restoreState()
    def draw_later_pages(canvas, doc):
        ...
        canvas.rect(0, 0, PAGE_WIDTH, PAGE_HEIGHT, fill=1, stroke=0)
        canvas.restoreState()
    sidebar_frame = Frame(45, 45, 145, PAGE_HEIGHT - HEADER_HEIGHT - 100, showBoundary=0, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
    main_frame_first = Frame(230, 45, PAGE_WIDTH - 270, PAGE_HEIGHT - HEADER_HEIGHT - 60, showBoundary=0, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
    later_frame = Frame(55, 45, PAGE_WIDTH - 110, PAGE_HEIGHT - 70, showBoundary=0, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
    first_template = PageTemplate(id='First', frames=[sidebar_frame, main_frame_first], onPage=draw_first_page)
    later_template = PageTemplate(id='Later', frames=[later_frame], onPage=draw_later_pages)
    ...
    def build_main_content():
        ...
        for exp in resume["experience"]:
            ...
            for project in exp["projects"]:
                content.append(Paragraph(f"{project['title']} ({project['duration']})", company_style))
...
def format_resume():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            return jsonify({
                "status": "success",
                "message": f"Resume formatted successfully for {resume['name']}!",
